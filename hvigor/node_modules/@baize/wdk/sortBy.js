...
function sortBy(collection) {
    ...
    if (iteratees == null || iteratees.length === 0) {
        ...
        for (var i = 0; i < len; i++) {
            if (Array.isArray(iteratees[i])) {
                // 如果传入是一个数组，例如 sortBy(col, ['a'], ['b'])，把它变成 ['a', 'b'];
                calcIteratee.push.apply(calcIteratee, __spreadArray([], __read(iteratees[i]), false));
            }
            else if (iteratees[i] == null) {
                // 清理其中的 null/undefined 迭代器，全部换成 entity
                calcIteratee.push(identity_1.default);
            }
            else {
                calcIteratee.push(iteratees[i]);
            }
        }
    }
