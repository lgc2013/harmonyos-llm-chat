---
description: 根据可用的设计工件为该功能生成可执行的、依赖排序的 tasks.md
handoffs: 
  - label: 分析一致性
...
1. **设置**：从仓库根目录运行 `.specify/scripts/powershell/check-prerequisites.ps1 -Json` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于参数中的单引号，如 "I'm Groot"，请使用转义语法：例如 'I'\''m Groot'（如果可以的话也可以用双引号："I'm Groot"）。
...
4. **生成 tasks.md**：使用 `.specify/templates/tasks-template.md` 作为结构，填充：
   ...
   - 每个阶段包括：故事目标、独立测试标准、测试（如有要求）、实施任务
   - 最后阶段：完善和横向关注点
...
3. **[P] 标记**：仅当任务可并行化时才包含（不同文件，依赖于未完成的任务）
...
2. **来自契约**：
   - 将每个接口契约 → 映射到它服务的用户故事
   - 如果需要测试：每个接口契约 → 在该故事阶段实施之前的契约测试任务 [P]
...
- **阶段 3+**：按优先级顺序排列的用户故事（P1、P2、P3...）
  - 在每个故事内：测试（如有要求）→ 模型 → 服务 → 端点 → 集成
